...
from utils import imshow, unloader
...
def create_nst_model(pretrained_vgg19_model,
                     content_img, 
                     style_img,
                     normalization_mean,
                     normalization_std,
    ...
    for i, layer_i in enumerate(pretrained_vgg19_model.features.children()):
        ...
        if (len(style_loss_layers) == len(style_layers)) and (len(content_loss_layers) == len(content_layers)):
            break
...
def run_style_transfer(pretrained_vgg19_model,
                       content_img,
                       style_img,
                       input_img=None,
                       save_folder=None,
    ...
    if input_img is None:
        input_img = torch.randn(size=content_img.size(), requires_grad=True, device=device)
        ...
        def closure():
            ...
            if run[0] % 50 == 0:
                ...
                if return_history_loss:
                    global history_loss
                    history_loss.append((all_content_loss.item(), all_style_loss.item()))
